...
class Routers extends Widget
{
    ...
    protected function normalizeItems()
    {
        foreach ($this->items as &$item) {
            ...
            foreach ($item['routers'] as &$router) {
                if (!isset($router['options'])) {
                    $router['options'] = [];
                }

                if (!isset($router['label'])) {
                    $router['label'] = '';
                } elseif ($this->encodeLabels) {
                    $router['label'] = Html::encode($router['label']);
                }

                if (!isset($router['icon'])) {
                    $router['icon'] = '';
                }

                if (!isset($router['url']) && !isset($router['route'])) {
                    $router['route'] = '';
                }
            }
        }
